...
class GoldenMatcherGenerator {
  ...
  const GoldenMatcherGenerator({required this.folder, this.extension = 'png'});
...
class EnhancedLocalFileComparator extends LocalFileComparator {
  ...
  Future<String> generateFailureOutput(
    ...
    return captureFailure
        ? _generateFailureOutputByCapturingFailure(
            result,
            golden,
            basedir,
            key: key,
  ...
  Future<String> _generateFailureOutputByCapturingFailure(
    ...
    return TestAsyncUtils.guard<String>(() async {
      ...
      for (final entry
        ...
        final pngBytes = await entry.value.toByteData(
...
Future<MyComparisonResult> _compareListsGivenSizeDiffer(
  ...
  final secondResult = await _compareListsWithExtraOutput(
    testTarget,
    masterTarget,
  );
...
Future<MyComparisonResult> compareUiImages(
  ...
  final pixelDiffHistogram = SimpleHistogram(
    totalSampleCount: totalPixels,
  ); // NOTE MODIFIED ADD
    ...
    for (int y = 0; y < height; y++) {
      ...
      if (diffPixel != 0) {
        ...
        final int maskPixel = _toRGBA(
          max(_toABGR(invertedMasterPixel), _toABGR(invertedTestPixel)),
        );
...
class GoldenFailureInfo {
  ...
  GoldenFailureInfo({required this.images});
...
extension ExtGoldenConfig on GoldenConfig {
  ...
  bool check(MyComparisonResult result) {
    ...
    for (final entry in greaterThanToleration ?? <GoldenTolerationEntry>[]) {
      final sampleCountForThresh = pixelDiffHistogram.sampleCountGreaterThan(
        entry.diffPerPixel,
      );
